...
for line in lines:
    ...
    if ruta_match:
        ...
        ruta = 'Todas'
    # Corrección de ortografía y normalización
    texto = line.lower()
    if 'servicio normal' in texto:
        ...
        concepto = 'Otro'
        estado = 'Otro'
    data.append({'ruta': ruta, 'concepto': concepto, 'estado': estado, 'reporte': line})

df = pd.DataFrame(data)

# Eliminar duplicados: solo el último reporte de cada ruta
df_limpio = df.sort_values('reporte').drop_duplicates('ruta', keep='last')
